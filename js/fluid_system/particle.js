...
var rgbdParticle = function(gl,xt,yt,zt) {
...
var Fluid = function() {
    ...
    this.solver = function(index,gl,global_time_count,stop_flag,rgbd_solve) {
        ...
        if(global_time_count<10000){
            ...
            if(rgbd_solve==1){
                if(this.rgbd_set==0){
                    this.rgbd= new rgbdParticle(gl,0,0,0);
    ...
    this.rgbd_solver = function(dt){
        first_thirty+=1;
        //console.log(first_thirty);

        if(first_thirty<24050){

            this.rgbd.velocity[1]= this.rgbd.velocity[1]+ this.GRAVITY[1]*0.01;

            this.rgbd.positions[0]= 0;
            this.rgbd.positions[1]= this.rgbd.positions[1]+ 0.0002*this.GRAVITY[1]*dt;
            this.rgbd.positions[2]= 0;
            
        }else{
            var cap= Math.min(this.my_neighbor[100].length,25);
            for(var i=0;i<cap;i++){
                this.rgbd.velocity[0]= this.rgbd.velocity[0]+ this.particles[this.my_neighbor[100][i]].velocity[0];
                this.rgbd.velocity[1]= this.rgbd.velocity[1]+ this.particles[this.my_neighbor[100][i]].velocity[1];
                this.rgbd.velocity[2]= this.rgbd.velocity[2]+ this.particles[this.my_neighbor[100][i]].velocity[2];
            }
            this.rgbd.velocity[0]= this.rgbd.velocity[0]/cap;
            this.rgbd.velocity[1]= this.rgbd.velocity[1]/cap;
            this.rgbd.velocity[2]= this.rgbd.velocity[2]/cap;


            this.rgbd.positions[0]= this.rgbd.positions[0]+ 0.001*this.rgbd.velocity[0]*dt;
            this.rgbd.positions[1]= this.rgbd.positions[1]+ 0.001*this.rgbd.velocity[1]*dt;
            this.rgbd.positions[2]= this.rgbd.positions[2]+ 0.001*this.rgbd.velocity[2]*dt;
        }
    ...
    this.rgbd_to_grid = function(flag){
        ...
        var tx= this.rgbd.positions[0];
        var ty= this.rgbd.positions[1];
        var tz= this.rgbd.positions[2];
    ...
    this.pci_converge = function(index,dt){
        ...
        for(var it=0;it<max_iteration;it++){
            ...
            //compute new ru diff
            var n_ru_diff= this.particles[index].density-static_water_ru;
    ...
    this.diffuse =function(dt){
        ...
        for(var i=0;i<this.emit_size;i++){
            var diff=0.0;
            diff= this.particles[i].density;
            var cap= Math.min(this.my_neighbor[i].length,25);
                for(var j=0;j<cap;j++){
                    diff= diff +fluid.particles[this.my_neighbor[i][j]].density;
                    //console.log();
                }
                diff= diff- cap*this.particles[i].density;

                var a= 0.01*0.01*this.numParticles_perLoop*this.numParticles_perLoop*diff;
                this.particles[i].density= this.particles[i].density + a;
                //console.log(diff);
        }
    }
